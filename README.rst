...
Salt Stack Formula to set up and configure sysvinit, an init system
...
Some states/ commands may refer to templates which aren't included in the files folder (``sysvinit/files``). Take a look at ``contrib/`` (if present) for e.g. template examples and place them in separate file roots (e.g. Git repository, refer to `GitFS <http://docs.saltstack.com/topics/tutorials/gitfs.html>`_) in your Salt master config.
